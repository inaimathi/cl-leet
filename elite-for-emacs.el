...
(defun elite-for-emacs ()
  ...
  (let ((buffer))
	...
	(progn (switch-to-buffer elite-for-emacs-buffer-name))
      (progn (random t)
  ...
  (let ((cmd)
	...
	(progn
	  ...
	  (if (eq this-command 'self-insert-command)
	      (progn (setq elite-for-emacs-command (concat elite-for-emacs-command (list event)))))
	  ...
	  (if (eq this-command 'previous-line)
	      (progn (setq modifiers (event-modifiers last-input-event))
		     (if (eq (car modifiers) 'shift)
			 (progn (if (> (length elite-for-emacs-command-history) 0)
				    (progn (goto-char (point-max))
					   (beginning-of-line)
					   (kill-line)
					   (insert (concat (elite-for-emacs-set-prompt)))
...
(defun elite-for-emacs-pre-command-hook ()
  ...
  (let ((cmd)
    ...
    (if (eq this-command 'self-insert-command) (progn))
...
(defun elite-for-emacs-default-frame-title ()
  ...
  (let () "Simple Shell Title"))

(defun elite-for-emacs-default-mode-line ()
  "Sets custom mode line"
  (let () (list "---" 'elite-for-emacs-command "-%-")))
